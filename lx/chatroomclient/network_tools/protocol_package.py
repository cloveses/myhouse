...
def ptl_dealheader(replacebodylen):
    header = ptl_header
    header = header[0:bgpos_version] + ptl_clientvesion\
             + header[bgpos_version + len(ptl_clientvesion):]
    header = header[0:bgpos_bodylen] + replacebodylen \
             + header[bgpos_bodylen + len(replacebodylen):]
    header = header[0:bgpos_retention] + ptl_retention \
             + header[bgpos_retention + len(ptl_retention):]
...
其中V代表实际版本号，值自己确定但要以V结束；C代表包内容（body)实际长度值，
值自己确定但要以C结束；R代表保留字段，值自己确定但要以R结束；

3.样例：
header = "1.0VVV78CCCCCCCCRRRRRRRRRR" 。我们自己组成包头（header)共26个字符，
其中版本号为"1.0"，包内容（body）长度为"78"，保留字段为""。
